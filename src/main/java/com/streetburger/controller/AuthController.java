...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<AuthResponse>> signup(@RequestBody SignupRequest request) {
        ...
        AuthResponse response = new AuthResponse(token, user.getId(), user.getPhoneNumber(), user.getName(),
                user.getRole().name());
    ...
    public ResponseEntity<ApiResponse<AuthResponse>> login(@RequestBody LoginRequest request) {
        return userRepository.findByPhoneNumber(request.getPhoneNumber())
                .map(user -> {
                    ...
                    if (matches) {
                        String token = jwtUtil.generateToken(user.getPhoneNumber(), user.getRole().name());
                        AuthResponse response = new AuthResponse(token, user.getId(), user.getPhoneNumber(),
                                user.getName(),
                                user.getRole().name());
